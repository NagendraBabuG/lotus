...
def run_tests(test_file, module_name, target_module, module_path):
    try:
        ...
        if os.path.exists(module_path):
            with open(module_path, 'r', encoding='utf-8') as f:
                print(f"Content of {module_path}:\n{f.read()}\n")
        log_result(module_name, target_module, False, "FAIL", f"Pytest failed with return code {e.returncode}: {e.stderr}")
        return False
    except Exception as e:
        print(f"Error running tests for {test_file}: {e}")
        log_result(module_name, target_module, False, "FAIL", f"Unexpected error: {e}")
        return False
...
summary = "\nAll SRC→REF tests passed." if all_tests_pass else "\nSome SRC→REF tests failed or were skipped. Check 'tests_res.txt' for details."
print(summary)
with open(RESULT_LOG, 'a', encoding='utf-8') as f:
    f.write("=" * 60 + "\n")
    f.write(summary + "\n")